...
export function CodeEditor({
  ...
  return (
    <div ref={editorRef} className={`rounded-xl bg-snuts-code border border-snuts-border overflow-hidden ${className}`}>
      {(fileName || language) && (
        <div className="flex items-center gap-3 px-4 py-2 bg-snuts-surface-3 border-b border-snuts-border">
          {fileName && <span className="text-snuts-muted font-code text-xs">{fileName}</span>}
          {language && <span className="text-snuts-cyan font-code text-xs">{language}</span>}
        </div>
      )}
                  ...
                  (() => {
                    ...
                    return parts.map((part, pi) => (
                      <span key={part} className="flex items-center gap-0">
                        ...
                        {part && <span className="text-snuts-text whitespace-pre">{part}</span>}
                      </span>
                    ))
                  })()
                ) : (
                  <span className="text-snuts-text whitespace-pre">{line.text || '\u00A0'}</span>
                )}
              </div>
            </div>
          )
